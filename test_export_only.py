#!/usr/bin/env python3
"""
Test Real Implementation - EXPORT ONLY

This script tests the real implementation with actual API calls for EXPORT containers only:
1. Get booking number from container
2. Move type = DROP FULL (always for EXPORT)
3. Check appointments with booking number
"""
...
def test_export_containers():
    """Test EXPORT containers only"""
    print("=" * 80)
    print("  EXPORT CONTAINER TEST - WITH ACTUAL API CALLS")
    print("=" * 80)
    ...
    print(f"[INFO] Found {len(export_containers)} EXPORT containers")
    ...
    query_folder = os.path.join('storage', 'users', str(user.id), 'emodal', 'queries', 'test_export_only')
    os.makedirs(query_folder, exist_ok=True)
    
    # Test each EXPORT container
    print("\n" + "=" * 80)
    print(f"  TESTING {len(export_containers)} EXPORT CONTAINERS")
    print("=" * 80)
    
    results = []
    
    for idx, row in export_containers.iterrows():
        export_data = row.to_dict()
        export_number = export_data.get('Container #', '')
        
        print(f"\n[{idx+1}/{len(export_containers)}] Testing EXPORT container: {export_number}")
        ...
        try:
            ...
            print(f"\n[RESULT] Container: {export_number}")
            ...
            results.append({
                'container': export_number,
                'success': result_export.get('success'),
                'booking_number': result_export.get('booking_number'),
                'terminal': result_export.get('terminal'),
                'move_type': result_export.get('move_type'),
                'available_slots': len(result_export.get('available_times', [])),
                'error': result_export.get('error', '')
            })
                
        except Exception as e:
            print(f"[ERROR] Test failed for {export_number}: {e}")
            results.append({
                'container': export_number,
                'success': False,
                'booking_number': None,
    ...
    print(f"\n[SUMMARY] Total EXPORT containers tested: {total_count}")
    print(f"[SUMMARY] Successful: {success_count}")
    print(f"[SUMMARY] Failed: {total_count - success_count}")
    
    print(f"\n[SUMMARY] Results by container:")
    for r in results:
        status = "[OK]" if r['success'] else "[FAIL]"
        print(f"  {status} {r['container']}")
        if r['booking_number']:
            print(f"       Booking: {r['booking_number']}")
            print(f"       Terminal: {r['terminal']}")
            print(f"       Move Type: {r['move_type']}")
            print(f"       Slots: {r['available_slots']}")
        if r['error']:
...
def main():
    """Main function"""
    print("\n" + "=" * 80)
    print("  EXPORT CONTAINER TEST UTILITY")
    print("=" * 80)
    print("\n[INFO] This will test EXPORT containers only (IMPORT skipped)")
    print("[WARNING] This will make REAL API calls to E-Modal!")
    print("[WARNING] This may take several minutes.")
    print("\n[INFO] Starting test...")
    
    test_export_containers()
